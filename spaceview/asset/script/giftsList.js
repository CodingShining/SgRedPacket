		...
		if(data.code == 1){
			...
			if(recharList.length){
				for(var i=0;i<recharList.length;i++){
					$("#rechargeListUl").append('<li class="giftsItem" data-contId="'+recharList[i].id+'"><h1>转账</h1><div class="detailListTitle"><span>数量</span><span>状态</span><span>时间</span></div><div class="detailListValue"><span>'+recharList[i].amount+'</span><span>已完成</span><span>'+SwitchTime(recharList[i].createtime)+'</span></div></li>');
				}
				//为所有的充值记录添加事件
